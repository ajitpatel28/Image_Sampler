    ...
    T: PixelExt<Component = u8>,
{
 ...
 bottom line
        ...
        if let Some(&k) = coeffs.get(y as usize) {
            ...
            let source1 = simd_utils::loadu_si256(components, src_x);
            let source2 = _mm256_setzero_si256();
    ...
    for dst_chunk in &mut dst_chunks_8 {
        ...
        for src_rows in src_img.iter_2_rows(y_start, max_y) {
            ...
            let source1 = simd_utils::loadl_epi64(components1, src_x);
            let source2 = simd_utils::loadl_epi64(components2, src_x);
        ...
        if let Some(&k) = coeffs.get(y as usize) {
            ...
            let source1 = simd_utils::loadl_epi64(components, src_x);
            let source2 = _mm_setzero_si128();
    ...
    if let Some(dst_chunk) = dst_chunks_4.next() {
        ...
        for src_rows in src_img.iter_2_rows(y_start, max_y) {
            ...
            let row1 = simd_utils::mm_cvtsi32_si128_from_u8(components1, src_x);
            let row2 = simd_utils::mm_cvtsi32_si128_from_u8(components2, src_x);
